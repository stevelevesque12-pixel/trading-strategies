...
from donchian_macd_vol.strategy import Signal

from backtest.donchian_macd_vol_engine import DonchianMacdVolBacktestEngine
...
def sample_csv(tmp_path_factory):
    ...
    df = generate(days=20, seed=23, start_price=5000.0, tz="America/New_York")
...
def test_backtest_runs_end_to_end(sample_csv, timeframe):
    ...
    engine = DonchianMacdVolBacktestEngine(timeframe=timeframe, instrument=instrument, risk=risk)
...
class _FireOnceStrategy:
    ...
    real Donchian/MACD/volume detection in DonchianMacdVolumeStrategy.
    """
...
def test_position_never_survives_a_data_gap_spanning_the_flatten_cutoff(tmp_path):
    ...
    engine = DonchianMacdVolBacktestEngine(timeframe="1min", instrument=instrument, strategy=strategy)
...
def test_risk_manager_caps_daily_trade_count(sample_csv):
    ...
    engine = DonchianMacdVolBacktestEngine(timeframe="5min", instrument=instrument, risk=risk)

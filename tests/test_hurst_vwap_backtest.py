...
from hurst_vwap.strategy import Signal

from backtest.hurst_vwap_engine import HurstVWAPBacktestEngine
...
def sample_csv(tmp_path_factory):
    ...
    df = generate(days=15, seed=13, start_price=5000.0, tz="America/New_York")
...
def test_backtest_runs_end_to_end(sample_csv):
    ...
    engine = HurstVWAPBacktestEngine(instrument=instrument, risk=risk)
...
class _FireOnceStrategy:
    ...
    real VWAP-band/Hurst detection in HurstVWAPStrategy.
    """
...
def test_position_never_survives_a_data_gap_spanning_the_flatten_cutoff(tmp_path):
    ...
    engine = HurstVWAPBacktestEngine(instrument=instrument, strategy=strategy)

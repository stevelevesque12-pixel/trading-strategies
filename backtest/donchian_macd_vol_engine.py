"""Event-driven backtest engine for the Donchian+MACD+Volume breakout strategy.

Single-timeframe, same shape as OverextensionBacktestEngine/ORBBacktestEngine
(both stop and target are static once a signal fires, so this reuses the
identical position management -- fill latency, stop/target-hit ordering,
session flatten, holiday-gap handling; see BacktestEngine's docstring,
backtest/engine.py, for the reasoning).
"""
...
class DonchianMacdVolBacktestEngine:
    def __init__(
        self,
        timeframe: str,
        instrument: Instrument,
        strategy: Optional[DonchianMacdVolumeStrategy] = None,
        ...
        self.strategy = strategy or DonchianMacdVolumeStrategy(tick_size=instrument.tick_size)

"""Event-driven backtest engine for the Hurst-gated VWAP band-fade strategy.

Single-timeframe, same shape as OverextensionBacktestEngine/ORBBacktestEngine:
stop/target are frozen at signal time, so this reuses the identical
static-stop/target position management (fill latency, stop/target-hit
ordering, session flatten, holiday-gap handling) -- see BacktestEngine's
docstring (backtest/engine.py) for the reasoning.

The Hurst window needs a literal 30-*minute* history, so this engine
always runs at 1-minute resolution (no --timeframe choice, unlike the
other single-timeframe engines).
"""
...
class HurstVWAPBacktestEngine:
    def __init__(
        self,
        instrument: Instrument,
        strategy: Optional[HurstVWAPStrategy] = None,
        risk: Optional[RiskManager] = None,
        session: Optional[SessionConfig] = None,
        contracts: int = 1,
    ):
        self.instrument = instrument
        self.strategy = strategy or HurstVWAPStrategy(tick_size=instrument.tick_size)
    ...
    def run(self, csv_path: str) -> List[Trade]:
        ...
        df = load_1m_csv(csv_path, tz=self.session.tz)

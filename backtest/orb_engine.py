"""Event-driven backtest engine for the ORB (Opening Range Breakout) strategy.

Single-timeframe, same shape as OverextensionBacktestEngine (backtest/overextension_engine.py):
no separate bias timeframe, so this walks one resampled bar series directly.
Position management (fill latency, stop/target-hit ordering, session
flatten, holiday-gap handling) mirrors BacktestEngine exactly -- see its
docstring for the reasoning. ORBStrategy's own entries are already
resting-stop-style (fired at the exact trigger level on the bar that
crosses it, not at that bar's close) -- the engine treats whatever
entry_price the strategy returns as the fill price, same as it does for
...
class ORBBacktestEngine:
    def __init__(
        ...
        strategy: Optional[ORBStrategy] = None,
        ...
        self.strategy = strategy or ORBStrategy(tick_size=instrument.tick_size)
